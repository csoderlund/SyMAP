...
public class GeneAlignment implements Comparable<GeneAlignment> {
	...
	private int eStart, eEnd; // only show exons in selected/hit area
	private Exon[] exons;
	...
	protected GeneAlignment(String name, int start, int end, boolean forward, Vector<Annotation> e) {
		if (name != null)
		...
		this.gStart = start;
		this.gEnd = end;
		this.isReverse = (start>end); // in DB, start<end, so always false
	...
	protected void setDisplayExons(int sstart, int send) { // selected/hit start and end
	try {
	...
	protected void setNoShow() {bShow=false;}

	// CloseUpComponent: startG, endG, HORZ_BORDER, bpPerPixel, (layers[i] * layerSize + y)
	protected void setBounds(int startGr, int endGr, double pixelStart, double bpPerPixel, double y) {
	...
	protected void paint(Graphics2D g2, FontMetrics fm, Color fontColor, double vertFontSpace) {
		if (!bShow) return;
	...
	private boolean paintGene(Graphics2D g2, FontMetrics fm, Color fontColor, double vertFontSpace) {
		...
		Color c = (isForward) ? CloseUpComponent.exonColorP : CloseUpComponent.exonColorN; 
		g2.setPaint(c);
	...
	protected double getWidth(double bpPerPixel, int minStartBP, int maxEndBP) {
		return getExonLength(minStartBP,maxEndBP) / bpPerPixel;
	}

	protected double getX(int startG, double pixelStart, double bpPerPixel) {
		return (getStartExon(startG)-startG)/bpPerPixel + pixelStart;
	}
	
	private double getgX(int startGr, double pixelStart, double bpPerPixel) {
		int start = Math.max(!isReverse ? gStart : gEnd,  startGr);
		return (start-startGr)/bpPerPixel + pixelStart;
	}
	private int getGeneLength(int paintStart, int paintEnd) {
		int end =   Math.min(!isReverse ? gEnd   : gStart, paintEnd);
		int start = Math.max(!isReverse ? gStart : gEnd,  paintStart); 
		return Math.max(end-start, 0);
	}
	
	protected int getGmin() { return isReverse  ? gEnd : gStart; }
	protected int getGmax() { return isReverse  ? gStart : gEnd; }
	protected int getEmin() { return isReverse  ? eEnd : eStart; }
	protected int getEmax() { return isReverse  ? eStart : eEnd; }
	 ...
	 */
	protected class Exon implements Comparable<Exon> {
		private int start;
		private int end;
		private String tag;
		private boolean bShow=true;

		protected Exon(int start, int end, String tag) {
			if (start > end) {
				int temp = start;
				start = end;
				end = temp;
		...
		private double getStart(int startBP, double bpPerPixel) {
			if (start <= startBP) return 0;
			return (start - startBP)/bpPerPixel;
		}
		private double getWidth(int startBP, int endBP, double bpPerPixel) {
			int x = (end > endBP) ? endBP : end;
			int y = (start < startBP) ? startBP : start;
			return Math.max((x-y)/bpPerPixel, 0);
		}
		private double getEnd(int startBP, int endBP, double bpPerPixel) {
			if (end < startBP) return 0;
			int x = (end > endBP) ? endBP : end;
			return (x - startBP)/bpPerPixel;
		}
		private Exon translate(int s, int e) {
			if (s <= e) return this;
			start = s-end+e; 
			end = s-start+e;
			return this;
	 ...
	 * ExonBox; 
	 */
	protected class ExonBox implements Shape {
		private DoublePolygon poly;

		private ExonBox() {poly = new DoublePolygon();}
