...
public class BlockViewFrame extends JFrame
{
	...
	private final int chromWidth = 8;
	private final int layerWidth = 8;
	private final int chromSpace = 25;
	private int blockMaxHeight = 700;
	...
	private boolean chromLabelVert = false;
	private boolean unorderedRef = false;
	private boolean unordered2 = false;
	...
	private void init() {		
		try {
			...
			Dimension d = new Dimension(blockWindowWidth + 50, blockMaxHeight + chromLabelOffset + 150);
			setPreferredSize(d); 
			setMinimumSize(d); // without this one, panel is not all shown, but can be resized
	
			topPane = new JPanel();
			topPane.setBackground(Color.white);
			topPane.setLayout(new GridBagLayout()); 
			topPane.setPreferredSize(d); topPane.setMinimumSize(d);
			...
			keyRow.add(new JLabel(Name2 + " color key"));
			topPane.add(keyRow,c);
			...
			d = new Dimension(blockTotalWidth, blockMaxHeight + chromLabelOffset + 10);
